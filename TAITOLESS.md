...
Deploy database migrations with Sqitch:
    ...
    sqitch -h localhost -p 6077 -d $db_database_name -u $db_database_app_username -f database/sqitch.plan deploy

Import development data to database:

    psql -h localhost -p 6077 -d $db_database_name -U $db_database_app_username -f database/data/local.sql
...
Open admin GUI in browser:
    ...
    http://localhost:9854/admin

Open server API in browser:

    http://localhost:9854/api/healthz

Open www site in browser:

    http://localhost:5269/docs

Connect to database using password `secret1234`:

    psql -h localhost -p 6077 -d $db_database_name -U $db_database_app_username
